...
class SafeCommandCharacteristic(Characteristic):
    ...
    ALLOWED_COMMANDS = {"date": "/bin/date", "uptime": "/usr/bin/uptime", "temp": "/usr/bin/vcgencmd measure_temp", "hostname": "/bin/hostname", "disk": "/bin/df -h /", "memory": "/usr/bin/free -h", "reboot": "sudo /usr/sbin/reboot"}
    ...
    def WriteValue(self, value, options):
        ...
        if command in self.ALLOWED_COMMANDS:
            try:
                result = subprocess.check_output(self.ALLOWED_COMMANDS[command], shell=True, timeout=5).decode("utf-8").strip()
...
class DockerContainerControlCharacteristic(Characteristic):
    ...
    def _get_container_status(self):
        ...
        rc, stdout, stderr = self._run(["docker", "inspect", "-f", "{{.Name}}|{{.State.Status}}|{{.State.Running}}|{{.Id}}", DOCKER_CONTAINER_NAME])
        if rc == 0 and stdout:
        ...
        rc2, stdout2, stderr2 = self._run(["docker", "ps", "-a", "--filter", f"name=^{DOCKER_CONTAINER_NAME}$", "--format", "{{.Names}}|{{.Status}}|{{.ID}}"])
