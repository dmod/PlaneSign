        ...
        for line in cmd_output.split("\n"):
            ...
            if "BSS" in line and "(" in line:  # New network found
                if current_network.get("ssid"):  # Save previous network if it had an SSID
                    network = WiFiNetwork(ssid=current_network["ssid"], signal=current_network.get("signal", "N/A"), encrypted=current_network.get("encrypted", "no"))
...
def configure_wifi(credentials):
    try:
        ...
        subprocess.run(["sudo", "nmcli", "connection", "delete", ssid], stderr=subprocess.DEVNULL)  # Ignore errors if connection doesn't exist

        # Add new connection - different commands for open vs secured networks
        if password.strip():  # Secured network
            subprocess.run(["sudo", "nmcli", "connection", "add", "type", "wifi", "con-name", ssid, "ifname", "wlan0", "ssid", ssid, "wifi-sec.key-mgmt", "wpa-psk", "wifi-sec.psk", password], check=True)
        else:  # Open network
            subprocess.run(["sudo", "nmcli", "connection", "add", "type", "wifi", "con-name", ssid, "ifname", "wlan0", "ssid", ssid], check=True)
