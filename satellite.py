    ...
    while True:
        ...
        if drawtime==None or time.perf_counter()-drawtime>30:
            ...
            if response.status_code == requests.codes.ok:
                ...
                graphics.DrawText(sign.canvas, sign.font57, 1, 15, graphics.Color(200, 10, 10), datetime.strptime(closest["launchDate"], "%Y-%m-%d").strftime("%b %d %Y"))

                graphics.DrawText(sign.canvas, sign.font57, 1, 23, graphics.Color(60, 60, 160), "Dst:")
                for x in range(1,15):
                    sign.canvas.SetPixel(x, 23, 110, 90, 0)
                if closest["dist"]<100:
                    graphics.DrawText(sign.canvas, sign.font57, 1, 31, graphics.Color(60, 60, 160), "{0:.1f}".format(closest["dist"]))
                else:
                    graphics.DrawText(sign.canvas, sign.font57, 1, 31, graphics.Color(60, 60, 160), "{0:.0f}".format(closest["dist"]))
                
                graphics.DrawText(sign.canvas, sign.font57, 23, 23, graphics.Color(160, 160, 200), "Alt:")
                if dupeflag:
                    for x in range(23,37):
                        sign.canvas.SetPixel(x, 23, 110, 90, 0)
                close_alt = closest["satalt"]*KM_2_MI
                if close_alt < 10000:
                    graphics.DrawText(sign.canvas, sign.font57, 23, 31, graphics.Color(160, 160, 200), "{0:.0f}".format(close_alt))
                else:
                    graphics.DrawText(sign.canvas, sign.font57, 23, 31, graphics.Color(160, 160, 200), str(round(close_alt/1000))+"k")

                graphics.DrawText(sign.canvas, sign.font57, 44, 23, graphics.Color(20, 160, 60), "Vel:")
                graphics.DrawText(sign.canvas, sign.font57, 44, 31, graphics.Color(20, 160, 60), "{0:.1f}".format(closest["vel"]*KM_2_MI))
                ...
                #divider
                for y in range(32):
                    sign.canvas.SetPixel(64, y, 0, 0, 100)
                ...
                if len(clean_low_name)>12:
                    for x in range(126,128):
                        sign.canvas.SetPixel(x, 6, 100, 100, 100)
                graphics.DrawText(sign.canvas, sign.font57, 66, 15, graphics.Color(200, 10, 10), datetime.strptime(lowest["launchDate"], "%Y-%m-%d").strftime("%b %d %Y"))

                graphics.DrawText(sign.canvas, sign.font57, 66, 23, graphics.Color(60, 60, 160), "Dst:")
                if lowest["dist"]<100:
                    graphics.DrawText(sign.canvas, sign.font57, 66, 31, graphics.Color(60, 60, 160), "{0:.1f}".format(lowest["dist"]))
                else:
                    graphics.DrawText(sign.canvas, sign.font57, 66, 31, graphics.Color(60, 60, 160), "{0:.0f}".format(lowest["dist"]))
                
                graphics.DrawText(sign.canvas, sign.font57, 88, 23, graphics.Color(160, 160, 200), "Alt:")
                if not dupeflag:
                    for x in range(88,102):
                        sign.canvas.SetPixel(x, 23, 110, 90, 0)
                low_alt = lowest["satalt"]*KM_2_MI
                if low_alt < 10000:
                    graphics.DrawText(sign.canvas, sign.font57, 88, 31, graphics.Color(160, 160, 200), "{0:.0f}".format(low_alt))
                else:
                    graphics.DrawText(sign.canvas, sign.font57, 88, 31, graphics.Color(160, 160, 200), str(round(low_alt/1000))+"k")

                graphics.DrawText(sign.canvas, sign.font57, 109, 23, graphics.Color(20, 160, 60), "Vel:")
                graphics.DrawText(sign.canvas, sign.font57, 109, 31, graphics.Color(20, 160, 60), "{0:.1f}".format(lowest["vel"]*KM_2_MI))
